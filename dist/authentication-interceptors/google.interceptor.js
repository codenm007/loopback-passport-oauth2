"use strict";
// Copyright IBM Corp. 2020. All Rights Reserved.
// Node module: @loopback/example-passport-login
// This file is licensed under the MIT License.
// License text available at https://opensource.org/licenses/MIT
Object.defineProperty(exports, "__esModule", { value: true });
exports.GoogleOauthInterceptor = void 0;
const tslib_1 = require("tslib");
const core_1 = require("@loopback/core");
const rest_1 = require("@loopback/rest");
let GoogleOauthInterceptor = class GoogleOauthInterceptor {
    constructor(googleStrategy) {
        this.googleStrategy = googleStrategy;
    }
    value() {
        return async (invocationCtx, next) => {
            const requestCtx = invocationCtx.getSync(rest_1.RestBindings.Http.CONTEXT);
            const request = requestCtx.request;
            if (request.query['oauth2-provider-name'] === 'google') {
                return rest_1.toInterceptor(this.googleStrategy)(invocationCtx, next);
            }
            return next();
        };
    }
};
GoogleOauthInterceptor = tslib_1.__decorate([
    tslib_1.__param(0, core_1.inject('googleStrategyMiddleware')),
    tslib_1.__metadata("design:paramtypes", [Function])
], GoogleOauthInterceptor);
exports.GoogleOauthInterceptor = GoogleOauthInterceptor;
//# sourceMappingURL=google.interceptor.js.map